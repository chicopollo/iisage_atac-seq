...
####################################################################################
# Script: multi_species_atac_pipeline_v2.sh
...
#   - Dry-run mode
#   - Version tracking
#   - Integrated R-based downstream analysis (NEW)
#
# Usage:
#   Single species: sbatch --export=SPECIES_NAME=species_name multi_species_atac_pipeline_v2.sh
#   All species: sbatch --export=DEPLOY_ALL=true multi_species_atac_pipeline_v2.sh
#   Dry run: sbatch --export=SPECIES_NAME=species_name,DRY_RUN=true multi_species_atac_pipeline_v2.sh
#   Array mode: sbatch --array=1-N --export=SPECIES_NAME=species_name multi_species_atac_pipeline_v2.sh
#   Skip R analysis: sbatch --export=SPECIES_NAME=species_name,RUN_R_ANALYSIS=false multi_species_atac_pipeline_v2.sh
#
...
# Validate and load required modules
load_and_validate_modules() {
    log_message "Loading required modules"

    if [[ "$DRY_RUN" == "true" ]]; then
        log_message "[DRY RUN] Would load modules: foss/2022b, Trimmomatic, Bowtie2/2.5.1-GCC-12.2.0, SAMtools/1.17-GCC-12.2.0, MACS2/2.2.9.1-foss-2022b, deepTools"
        return 0
    fi
    ...
    # Modules that match the 2022b toolchain
    local modules=(
        "Trimmomatic"
    ...
    if [[ ${#failed_modules[@]} -gt 0 ]]; then
        log_message "ERROR: Failed to load modules: ${failed_modules[*]}"
        log_message "Available modules can be checked with: module spider <module_name>"
        exit 1
    fi

    # Verify key commands exist
    local commands=("trimmomatic" "bowtie2" "samtools" "macs2" "bamCoverage")
    ...
    log_message "All required modules and commands validated successfully"
}
...
record_versions() {
...

    {
        ...
        echo "--- Core Tools ---"
        trimmomatic -version 2>&1 | head -1 || echo "Trimmomatic: version unavailable"
        bowtie2 --version | head -1 || echo "Bowtie2: version unavailable"
        samtools --version | head -1 || echo "SAMtools: version unavailable"
        macs2 --version 2>&1 || echo "MACS2: version unavailable"
        bamCoverage --version 2>&1 | head -1 || echo "deepTools: version unavailable"
...
# Build or validate Bowtie2 index
setup_bowtie2_index() {
    local ref_fasta="$1"
    local ref_genome="$2"
