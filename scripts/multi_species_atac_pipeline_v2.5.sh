...
#   - Size selection moved to array processing stage for parallelization
#   - Array jobs now produce size-selected BAMs before exit
#   - Analysis job uses pre-computed size-selected BAMs (no re-computation)
#   - Added checkpoint/resume for size-selected BAMs
#   - Optimized two-job workflow (array → analysis)
#
# Improvements in v2.5:
#   - Peak calling moved to array processing stage for full parallelization
#   - 16 samples now call peaks in parallel instead of sequentially
#   - Massive speed improvement: ~16x faster peak calling
#   - Analysis job only runs multi-sample comparisons (deepTools, R)
#   - All per-sample operations now parallelized in array stage
#   - SAM file validation before resume (detects truncated/corrupted files)
#   - Smart resume from intermediate FASTQ files (trimmed, merged)
#   - Skips unnecessary merge/trim operations when files already exist
#
# Usage:
...
R_MODULE="${R_MODULE:-R/4.2.2-foss-2022b}"                    # R module name for SLURM
...
load_species_config() {
    ...
    # Use SLURM CPU allocation if available (overrides config)
    if [[ -n "${SLURM_CPUS_PER_TASK:-}" ]]; then
        export THREADS="$SLURM_CPUS_PER_TASK"
        log_message "Using SLURM allocation: $THREADS threads (overriding config)"
    else
        export THREADS="${THREADS:-$DEFAULT_THREADS}"
        log_message "Using config threads: $THREADS"
    fi
...
process_sample_array() {
    ...
    # Resume capability: Check for existing BAM file
    if [[ -f "$bam_file" ]] && [[ -f "${bam_file}.bai" ]]; then
        if [[ "$DRY_RUN" == "false" ]]; then
            if samtools quickcheck "$bam_file" 2>/dev/null; then
                log_message "Valid BAM file exists for $sample_name, skipping all processing"
                create_checkpoint "sample_${sample_name}_aligned"
                return 0
            else
                log_message "WARNING: Existing BAM file is corrupted, will reprocess"
                rm -f "$bam_file" "${bam_file}.bai"
            fi
        else
            log_message "[DRY RUN] Would use existing BAM file for $sample_name"
            return 0
        fi
    fi

    # Resume capability: Check for existing SAM file and validate it
    local resume_from_sam=false
    if [[ -f "$sam_file" ]] && [[ -s "$sam_file" ]]; then
        if [[ "$DRY_RUN" == "false" ]]; then
            # Validate SAM file by checking if it can be read completely
            if samtools view -H "$sam_file" > /dev/null 2>&1 && \
               samtools view "$sam_file" | tail -1 > /dev/null 2>&1; then
                log_message "Valid SAM file found for $sample_name, resuming from BAM conversion"
                resume_from_sam=true
            else
                log_message "WARNING: Existing SAM file is corrupted or incomplete, will re-align"
                rm -f "$sam_file"
                resume_from_sam=false
            fi
        else
            log_message "[DRY RUN] Would validate and possibly use existing SAM file"
            resume_from_sam=true
        fi
    fi
    ...
    if [[ "$RUN_TRIMMING" == "true" ]] && [[ "$RUN_ALIGNMENT" == "true" ]]; then
        ...
        if [[ "$resume_from_sam" == "false" ]]; then
            # Check for existing intermediate files to resume from
            local skip_merge=false
            local skip_trim=false

            # Check if trimmed paired FASTQs exist (most processed state)
            if [[ -f "$paired_r1" ]] && [[ -f "$paired_r2" ]] && \
               [[ -s "$paired_r1" ]] && [[ -s "$paired_r2" ]]; then
                log_message "Trimmed FASTQ files found for $sample_name, skipping merge and trim"
                skip_merge=true
                skip_trim=true
            # Check if merged FASTQs exist
            elif [[ -f "$merged_r1" ]] && [[ -f "$merged_r2" ]] && \
                 [[ -s "$merged_r1" ]] && [[ -s "$merged_r2" ]]; then
                log_message "Merged FASTQ files found for $sample_name, skipping merge"
                skip_merge=true
            fi
            ...
            # Alignment (always run if no valid SAM/BAM exists)
            ...
            cleanup_files "$paired_r1" "$paired_r2" "$unpaired_r1" "$unpaired_r2"
        fi

        # Convert to BAM, sort and index (always run if no valid BAM exists)
        ...
        # Cleanup SAM file after successful BAM creation
        cleanup_files "$sam_file"

        create_checkpoint "sample_${sample_name}_aligned"
        log_message "Successfully processed sample: $sample_name"
    fi

    # Size selection and filtering (run in array mode for parallelization)
    local size_selected_bam="${PEAKS_DIR}/${sample_name}_size_selected_${SIZE_SELECTION_CUTOFF}.bam"

    # Check if size-selected BAM already exists
    if [[ -f "$size_selected_bam" ]] && [[ -f "${size_selected_bam}.bai" ]]; then
        if [[ "$DRY_RUN" == "false" ]]; then
            if samtools quickcheck "$size_selected_bam" 2>/dev/null; then
                log_message "Valid size-selected BAM exists for $sample_name, skipping size selection"
                return 0
            else
                log_message "WARNING: Existing size-selected BAM is corrupted, will reprocess"
                rm -f "$size_selected_bam" "${size_selected_bam}.bai"
            fi
        fi
    fi
    ...
    log_message "Size selection complete for $sample_name"

    # Peak calling (run in array mode for parallelization)
    # Check if peaks already called
    if check_checkpoint "sample_${sample_name}_peaks"; then
        log_message "Peaks already called for $sample_name, skipping"
        return 0
    fi

    # Load peak calling modules
    load_peakcalling_modules
    ...
    log_message "Peaks called successfully for $sample_name"
}
...
##### STAGE 2: Verify Peak Calling Completion #####

if [[ "$RUN_PEAK_CALLING" == "true" ]]; then
    log_stage "Verifying Peak Calling Completion"

    # Peak calling happens in array mode, just verify outputs exist
    peak_count=$(find "$PEAKS_DIR" -name "*_peaks.narrowPeak" -type f 2>/dev/null | wc -l)

    if [[ $peak_count -eq 0 ]]; then
        log_message "ERROR: No peak files found in $PEAKS_DIR"
        log_message "Peak calling should have been completed during array job processing"
        exit 1
    fi

    log_message "Verified $peak_count peak files from array job processing"
    log_message "Peak files:"
    for peak_file in "$PEAKS_DIR"/*_peaks.narrowPeak; do
        if [[ -f "$peak_file" ]]; then
            sample_name=$(basename "$peak_file" _peaks.narrowPeak)
            peak_lines=$(wc -l < "$peak_file")
            log_message "  - $sample_name: $peak_lines peaks"
        fi
    done
fi
