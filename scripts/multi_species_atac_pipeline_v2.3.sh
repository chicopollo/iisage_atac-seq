...
process_sample_array() {
    ...
    # Resume capability: Check for existing SAM file
    local resume_from_sam=false
    if [[ -f "$sam_file" ]] && [[ -s "$sam_file" ]]; then
        log_message "Existing SAM file found for $sample_name, resuming from BAM conversion"
    ...
    if [[ "$RUN_TRIMMING" == "true" ]] && [[ "$RUN_ALIGNMENT" == "true" ]]; then
        ...
        if [[ "$resume_from_sam" == "false" ]]; then
            # Full processing: Merge → Trim → Align
