...
load_and_validate_modules() {
    ...
    if [[ "$DRY_RUN" == "true" ]]; then
        log_message "[DRY RUN] Would load modules: Trimmomatic, Bowtie2, SAMtools, MACS2, deepTools"
        return 0
    fi

    module purge

    local modules=("Trimmomatic" "Bowtie2" "SAMtools" "MACS2/2.2.9.1-foss-2022b" "deepTools")
